...
root=/raid/user_data/aravt/ssd/artifacts/final_wrapup_parallel_20260420/runs/ddtree/main/summary_artifacts
out=/raid/user_data/aravt/ssd/artifacts/final_wrapup_parallel_20260420/runs/ddtree/main/summary.json
...
for batch in 1 2 4; do
  for tb in 8 16; do
    for fc in 1 2; do
      for mode in ddtree ddtree_ssd_exact_off ddtree_ssd_exact_on_oracle ddtree_ssd_predicted_on_surrogate; do
        ...
        if ssh catalyst "[ -f '$adir/result.json' ]"; then
          ...
          ssh catalyst "cd $remote_root && export PATH=$remote_root/.venv/bin:/usr/local/bin:/usr/bin:/bin SSD_HF_CACHE=/raid/catalyst/models/hub SSD_DATASET_DIR=/raid/user_data/aravt/datasets CUDA_VISIBLE_DEVICES=6,7 SSD_DIST_PORT=$port; ./.venv/bin/python -O scripts/eval_ddtree.py --target '$target' --draft '$draft' --training-metadata '$meta' --output-len 32 --gpus 2 --max-prompts 78 --mode '$mode' --batch-size '$batch' --tree-budget '$tb' --frontier-count '$fc' --predictor '$predictor' --artifact-dir '$adir'"
        fi
        port=$((port + 1))
      done
    done
...
root = Path('/raid/user_data/aravt/ssd/artifacts/final_wrapup_parallel_20260420/runs/ddtree/main/summary_artifacts')
out = Path('/raid/user_data/aravt/ssd/artifacts/final_wrapup_parallel_20260420/runs/ddtree/main/summary.json')
batch_sizes = [1, 2, 4]
tree_budgets = [8, 16]
frontier_counts = [1, 2]
modes = ['ddtree', 'ddtree_ssd_exact_off', 'ddtree_ssd_exact_on_oracle', 'ddtree_ssd_predicted_on_surrogate']
